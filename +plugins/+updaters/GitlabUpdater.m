classdef GitlabUpdater < plugins.Updater
    ...
    methods (Hidden)
        function url = getURL(self, path, varargin)
            ...
            url = getURL@plugins.Updater(self, strcat(path, '?', querystring(2:end)));
        end
    ...
    methods
        function self = GitlabUpdater(varargin)
            self@plugins.Updater(varargin{:});

            % Create a configuration file where we will store the token and
            % other gitlab-specific information
            confile = fullfile(userdir(), '.gitlab.json');
            self.config = plugins.Configuration(confile);
    ...
    methods (Access = 'private')
        function fetchToken(self)
            ...
            msg = ['<html>Gitlab requires the use of a token ', ...
                   'to access the API.<br> You can create a ', ...
