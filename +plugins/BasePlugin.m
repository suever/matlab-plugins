classdef BasePlugin < hgsetget &  matlab.mixin.Heterogeneous
    % BasePlugin - Base class for all plugins
    %
    %   This is a bare-bones class that provides the API for all plugins
    %
    ...
    methods
        function self = BasePlugin(varargin)
            ...
            self.Config = plugins.Configuration(settingfile);
            ...
            if ~isfield(self.Config, 'updater') || ...
                ...
                self.Config.updater = plugins.structobj();
            elseif ~isa(self.Config.updater, 'plugins.structobj')
                self.Config.updater = plugins.structobj(self.Config.updater);
            end
        end
        ...
        function setStatus(self, message, varargin)
            ...
            eventData = plugins.StatusEvent('', message, varargin{:});
        ...
        function varargout = hasUpdate(self, callback)
            ...
            updater = plugins.Updater.create(self, 'Config', self.Config.updater);
    ...
    methods (Sealed)
        function [available, msg] = isAvailable(self, varargin)
            ...
            %
            % USAGE:
            %   [bool, msg] = obj.isAvailable(varargin)
            %
            % INPUTS:
